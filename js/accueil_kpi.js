...
options: {
    ...
    scales: {
        ...
        /***********************************************************
 * 11. Texte explicatif dynamique EXACTEMENT comme ta page
 ***********************************************************/

// --- 1. Construire une map année → valeurs (scope1&2 + aero)
...
// --- 2. Récupération des années clés
...
// --- 3. Évolution totale 2017–2024 (aéronefs + s12)
let YY = null;
if (total2017 && total2017 > 0 && total2024 !== null) {
    YY = ((total2024 / total2017) - 1) * 100;
}

// --- 4. Évolution des seuls scopes 1&2 (location-based)
let ZZ = null;
if (y2017 && y2017.s12 > 0 && y2024) {
    ZZ = ((y2024.s12 / y2017.s12) - 1) * 100;
}

// --- 5. % des scopes 1&2 dans les émissions de l’aéroport en 2026
// (calculé dans KPI 1 → aero + s12 + autres)
const XX = (typeof window._kpi1_shareS12 === 'number')
    ? window._kpi1_shareS12
    : null;
...
// --- 6. Texte sur les scopes 1&2 (ZZ)
let texteZZ;
if (ZZ === null) {
    texteZZ = `la variation des scopes 1 & 2 est « n.d. » faute de données complètes`;
} else if (ZZ < 0) {
    texteZZ = `ces émissions "Location based" ont été réduites de ${Math.abs(ZZ).toFixed(1)}% entre 2017 et 2024`;
} else if (ZZ > 0) {
...
// --- 8. Construction finale du texte
const explainHTML = `
<p>
...
// --- 9. Injection dans la DIV prévue
